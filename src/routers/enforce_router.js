...
import { playerCountChange } from '../utils/player/enforce.js';
...
const update_enforce_validation = joi.object({
  ...
  probability: joi.number().min(0).optional(),
});
...
enforce_router.patch('/enforce', authMiddleware, async (req, res, next) => {
  try {
    ...
    console.log('난수 : ' + number + ', probability : ' + rarity_row_enforce.probability);
    ...
    await playerCountChange(exist_hold_player);
    return res.status(201).json('강화에 성공했습니다.');
  } catch (error) {
    next(error);
  }
});
